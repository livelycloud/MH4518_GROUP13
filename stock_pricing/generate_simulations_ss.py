from multi_asset_GBM import MultiAssetGBM
import pandas as pd
import numpy as np
from tqdm import tqdm
import os
from scipy.stats import norm


if __name__ == "__main__":

    print(os.getcwd())

    Nsim = 1000
    np.random.seed(4518)
    save_path = "../generated_data/GMB_simple_one_year_window_1000_ss/" 
    time_format = "%Y-%m-%d"
    
    sample_cols = ["GOOGL", "MSFT", "AAPL"]
    fixing_date = "2023-07-26"
    asset_names = ["GOOGL UW Equity", "MSFT UW Equity", "AAPL UW Equity"]
    df = pd.read_csv("stock_historical_prices.csv")

    historical_data_start_date = "2021-07-01"
    
    multassetGBM = MultiAssetGBM(df, fixing_date, asset_names)
    multassetGBM.set_start_date(historical_data_start_date)

    for cur_date in tqdm(pd.bdate_range("2022-08-15", "2022-10-31")): # "2022-05-31", "2022-08-15", "2022-10-31"
        sample_start_date = cur_date + pd.Timedelta(days = 1)
        historical_data_start_date = (cur_date - pd.Timedelta(days = 365)).strftime(time_format)
        multassetGBM.set_start_date(historical_data_start_date)
        days = multassetGBM.get_days(cur_date.strftime(time_format))
        no_of_assets = multassetGBM.get_no_assets()
        samples = []
        for i in range(Nsim):
            w = np.empty((no_of_assets,days))
            for j in range(no_of_assets):
                for k in range(days):
                    u = np.random.uniform(0, 1)
                    V = i/Nsim + u/Nsim
                    w[j][k] = norm.ppf(V)
            sample = pd.DataFrame(multassetGBM.get_path_ss(cur_date.strftime(time_format),w).transpose(), 
                   columns = sample_cols, 
                   index = pd.bdate_range(sample_start_date, fixing_date, name = "date")) 
            samples.append(sample)
        for i in range(Nsim):
            samples[i]["index"] = i
        samples_df = pd.concat(samples)

        cur_path = save_path + sample_start_date.strftime(time_format) + ".csv"
        print(cur_path)
        samples_df.to_csv(cur_path)